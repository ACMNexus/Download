...
public class DownloadService extends Service {
    ...
    private static ExecutorService buildDownloadExecutor() {
        final int maxConcurrent = Resources.getSystem().getInteger(R.integer.config_MaxConcurrentDownloadsAllowed);
    ...
    private Handler.Callback mUpdateCallback = new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            ...
            if (DEBUG_LIFECYCLE) Log.v(Constants.TAG, "Updating for startId " + startId);
            ...
            if (msg.what == MSG_FINAL_UPDATE) {
                ...
                Log.wtf(Constants.TAG, "Final update pass triggered, isActive=" + isActive
            ...
            if (isActive) {
                ...
                if (stopSelfResult(startId)) {
                    if (DEBUG_LIFECYCLE) Log.v(Constants.TAG, "Nothing left; stopped");
                    getContentResolver().unregisterContentObserver(mObserver);
                    mScanner.shutdown();
                    mUpdateThread.quit();
                }
    ...
    private void deleteDownloadLocked(long id) {
        ...
        if (info.mDestination != Downloads.Impl.DESTINATION_EXTERNAL && info.mFileName != null) {
            if (Constants.LOGVV) {
                Log.d(Constants.TAG, "deleteDownloadLocked() deleting " + info.mFileName);
            }
            deleteFileIfExists(info.mFileName);
        }
    ...
    private void deleteFileIfExists(String path) {
        if (!TextUtils.isEmpty(path)) {
            if (Constants.LOGVV) {
                Log.d(Constants.TAG, "deleteFileIfExists() deleting " + path);
            }
            final File file = new File(path);
            if (file.exists() && !file.delete()) {
                Log.w(Constants.TAG, "file: '" + path + "' couldn't be deleted");
            }
        }
    }
